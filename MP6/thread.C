...
void Thread::setup_context(Thread_W_ARGS_Function _tfunction, void* arg){
    ...
    */
    ...
    push((unsigned long) arg); /* At this point we don't have arguments. */
...
Thread::Thread(Thread_W_ARGS_Function _tf, char * _stack, unsigned int _stack_size, void* arg) {
/* Construct a new thread and initialize its stack. The thread is then ready to run.
   (The dispatcher is implemented in file "thread_scheduler".) 
*/
